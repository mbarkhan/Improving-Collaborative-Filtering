...
public class AprioriCalculation extends SocialRecommender {
    ...
    public Vector aprioriProcess() {

        for (MatrixEntry matrixEntry : trainMatrix) {

            transaFile = matrixEntry.row() + "";
            configFile = matrixEntry.column() + "";
        ...
        try {
            ...
            for (int i = 0; i < candidates.size(); i++) {
                ...
                if ((count[i] / (double) numTransactions) >= minSup) {
                    if ((count[i] / (double) numTransactions) >= minConf) {
    ...
    @Override
    protected double predict(int i, int i1) throws LibrecException {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
}
